...
original_text_path = os.path.join(wd, "files", "รัฐธรรมนูญ2540_original.txt")
html_output_path = os.path.join(wd, "constitution_2525.html")
base_pdf_name = "รัฐธรรมนูญ_๒๕๒๕_แก้ไขเพิ่มเติม"
pdf_output_path = os.path.join(wd, "files", f"{base_pdf_name}.pdf")
if os.path.exists(pdf_output_path):
    try:
        ...
        v = 1
        while True:
            candidate = os.path.join(wd, "files", f"{base_pdf_name}_v{v}.pdf")
            if not os.path.exists(candidate):
                ...
                try:
                    ...
                    v += 1
garuda_img_path = os.path.join(wd, "images", "Thai_Garuda_emblem.svg")
...
with open(os.path.join(wd, "files", "processed_constitution.txt"), "w", encoding="utf-8") as f:
    f.write(text)
